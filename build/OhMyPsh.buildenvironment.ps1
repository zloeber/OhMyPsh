...
if ((Get-Variable 'BuildEnv' -ErrorAction:SilentlyContinue) -eq $null) {
    $Script:BuildEnv = New-Object -TypeName PSObject -Property @{
        ...
        ModuleVersion = '0.0.5'
        ModuleWebsite = 'https://github.com/zloeber/OhMyPsh'
        ModuleCopyright = "(c) $((get-date).Year.ToString()) Zachary Loeber. All rights reserved."
        ModuleLicenseURI = 'https://github.com/zloeber/OhMyPsh/LICENSE.md'
        ModuleTags = 'profile,console,shell,plugin,command_line' -split ','
        ...
        # You should NOT enter this key here but rather manually enter it in the OhMyPsh.buildenvironment.json file with: set-buildenvironment -NugetAPIKey '<key>'
    ...
    ########################################
    $PersistentBuildFile = join-path $PSScriptRoot "OhMyPsh.buildenvironment.json"
