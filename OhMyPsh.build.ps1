param (
    ...
    [string]$BuildFile = (Join-Path $BuildRoot 'build\OhMyPsh.buildenvironment.ps1'),
...
task BuildSessionCleanup {
    ...
    Write-Description White "Running post-build scripts" -Level 2
    $CleanupPath = Join-Path $BuildToolPath 'cleanup'
    Get-ChildItem -Path $CleanupPath -Recurse -Filter "*.ps1" -File | Foreach {
        Write-Description White "Dot sourcing cleanup script file: $($_.Name)" -Level 3
        . $_.FullName
    }
    if ($Script:BuildEnv.OptionTranscriptEnabled) {
